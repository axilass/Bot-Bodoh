exports.info = (id, Bot-Bodoh, corohelp, tampilTanggal, tampilWaktu, instagram, telegram, youtube, kapanbotaktif, grupch1, grupch2) => {
	return `⚜ ${Bot-Bodoh} ⚜
  
HALO *${id.split("@s.whatsapp.net")[0]}*
"
APA KABAR? SEMOGA BAIK-BAIK SAJA!
Sebelum menggunakan bot ini ada baiknya kalian melihat menu *!info* dan *!donasi* 😁

💵TOLONG BANTU DONASI AGAR BOT INI BISA TERUS BERJALAN💵
💵GUNAKAN COMMAND !donasi untuk berdonasi💵

INFO BOT! :
🔱 *AUTHOR*: Taufik
🔱 *GITHUB*: https://github.com/axilass/Bot-Bodoh
🔱 *INSTAGRAM*: ${https://www.instagram.com/axilas.game}
🔱 *YOUTUBE*: ${Taufik Sundoro}
🔱 *TELEGRAM* : ${https://t.me/Axilas12}
🔱 *WHATSAPP* : 088239762769

🔱 *SCRIPT ORIGINAL BY*: https://github.com/fdciabdul

INFO LAIN! :

BOT INI MENGGUNAKAN SOURCE CODE YANG SAYA DAPATKAN DAN SAYA MODIFIKASI DENGAN PENAMBAHAN, PENGURANGAN, DAN PERUBAHAN BEBERAPA FITUR.
BOT INI DIBUAT MENGGUNAKAN NODE.JS

THANKS TO: 
-ALLAH SWT
-MUHAMMAD SAW
-AYAH & IBU
-ABDUL MUTTAQIN(FDCIABDUL)
	


bot aktif selama : ${Diaktifkan}
JANGAN LUPA FOLLOW instagram
 ${https://www.instagram.com/axilas.game}

 👾TELEGRAM : ${https://t.me/Axilas12}`
}
